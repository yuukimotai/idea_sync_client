  ...
  useEffect(() => {
    if (ready) {
      apiCall<Idea>(`/api/ideas/${id}`)
        .then((result) => { setIdea(result); setTitle(result.title); setDescription(result.description); })
        .catch(() => setError("アイデアの取得に失敗しました"))
  ...
  return (
    <>
      ...
      <Container maxWidth="md" sx={{ py: 4 }}>
        <Typography variant="h4" sx={{ mb: 3 }}>アイデアを編集</Typography>
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ maxWidth: 500 }}>
          ...
          <Box sx={{ display: "flex", gap: 2, mt: 3 }}>
            ...
            <Button variant="outlined" component={Link} href={`/ideas/${id}`}>キャンセル</Button>
          </Box>
        </Box>
      </Container>
    </>
